...
export default {
  ...
  computed: {
    descriptionHtml () {
      // v-html에 연결된 함수로 descripton요소를 변환된 html코드로 바인드 처리
      if (!this.description) {
        return ''
      }
  ...
  mounted () {
    setTimeout(() => {
      // 모든 auto-size 요소의 자동 크기 조절 기능 초기화
      autosize($('.auto-size'))
    }, 0)
    // 카드 모달 창이 열리면
    $('#cardModal').on('show.bs.modal', () => {
      setTimeout(() => {
        // textarea의 높이를 필요한 크기로 조절하기 위해 autosize.update() 메소드 호출
        autosize.update($('.auto-size'))
      }, 0)
      this.loadActivities() // 활동내역 로드
      this.loadAttachments() // 첨부파일 내역 로드
    })
  },
  methods: {
    changeCardTitle () {
    ...
    onUploadingAttachment () {
      // 업로드되고 있는 파일이 있는지 추적하기 위해 uploadingCounter를 사용
      this.uploadingCount++
    },
    ...
    onAttachmentUploaded (attachment) {
      // 파일이 성공적으로 업로드되면
      this.uploadingCount--
      // 파일이 첨부파일 리스트에 나타나도록 파일을 첨부파일 배열에 추가
...
.modal {
  .modal-dialog {
    ...
    .modal-body {
      ...
      display: flex; // 하위의 박스를 수평으로 나란히 놓기 위해 flex를 사용
